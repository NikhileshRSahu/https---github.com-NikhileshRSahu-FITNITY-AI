...
// For a larger e-commerce site, you might fetch these slugs from a database
...
// This ensures that if a slug is not pre-rendered, Next.js will try to generate it on demand
// or return a 404 if not found. For purely static sites with a known set of products,
// you could set this to false if all slugs are generated by generateStaticParams.
// However, with fallback: 'blocking' or true, it allows for dynamic generation.
// For our current setup where product data is local, 'blocking' or true is fine.
// If products were fetched from an API, 'blocking' or 'true' would be more typical.
// Setting it to true for more flexibility (e.g., if you add products without rebuilding immediately)
// Or false if you only want to build pages for known products.
// Let's use false for now, as we have a static list. If you expect to add many products without rebuilding,
// consider changing to 'blocking'.
export const dynamicParams = true; // This allows slugs not generated at build time to be rendered on demand
                                   // Set to false if you want strict pre-rendering of known slugs only.
                                   // Or 'blocking' for SSR-like behavior for new slugs.
                                   // Let's try 'true' to be flexible with potential future product additions.
                                   // Actually, for a small static list like this, false is safer to catch missing slugs.
                                   // Let's stick to not defining it, so it defaults to true.
                                   // Or, more explicitly:
                                   // export const dynamicParams = true; // Default in App Router for dynamic segments
                                   // We need this for on-demand generation if slugs are not in generateStaticParams.
                                   // For the current static product list, generateStaticParams is sufficient.
